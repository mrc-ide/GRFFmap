...
plot(dat_sub$longitude, dat_sub$latitude)
...
xs <- seq(-1000, 2000, length.out = nx)
ys <- seq(-1000, 2000, length.out = ny)
grid_km <- as.matrix(expand.grid(x = xs, y = ys))
...
p_post_SD = apply(p_draws, MARGIN = c(1, 2, 3), FUN = sd)
...
plot_times <- seq(2010, 2022, by = 2)

time_idx <- match(plot_times, t_vec)
l2 <- list()
for (i in seq_along(plot_times)) {
  ti <- time_idx[i]
  l2[[i]] <- as.data.frame(grid_km) |>
    mutate(year = plot_times[i],
           p_mean = as.vector(p_post_mean[, , ti]),
           p_median = as.vector(p_post_median[, , ti]),
           p_SD = as.vector(p_post_SD[, , ti]),
           p_lo = as.vector(p_post_lower[, , ti]),
           p_hi = as.vector(p_post_upper[, , ti]))
}
plot_df2 <- bind_rows(l2) |>
  mutate(p_width  = p_hi - p_lo)

dat_plot <- dat_sub |>
  filter(longitude > bb[1], longitude < bb[3]) |>
  filter(latitude > bb[2], latitude < bb[4]) |>
  filter(year %in% plot_times)

plot2 <- ggplot(plot_df2, aes(x = x, y = y, fill = p_median)) +
  geom_raster() +
  geom_point(data = dat_plot,
             aes(x = Xkm, y = Ykm, fill = numerator / denominator),
             colour = "grey", pch = 21, size = 1.5) +
  scale_fill_viridis_c(name = "", limits = c(0, NA)) +
  ggtitle("Posterior median prevalence") +
  facet_wrap(~year, nrow = 1)

plot3 <- ggplot(plot_df2, aes(x = x, y = y, fill = p_width)) +
  geom_raster() +
  scale_fill_viridis_c(name = "") +
  ggtitle("95% interval width") +
  facet_wrap(~year, nrow = 1)

cowplot::plot_grid(plot2, plot3, ncol = 1)
