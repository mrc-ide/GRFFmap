...
# ============================================================
# Analytic posterior SD on logit scale from RTS smoother
# ============================================================
z_post_sd_analytic <- array(NA_real_, dim = c(nx, ny, t_num))

for (ti in 1:t_num) {
  PhiP           <- Phi_full %*% forceSymmetric(C_smooth_final[[ti]])
  z_var_vec      <- rowSums(PhiP * Phi_full)
  z_post_sd_analytic[, , ti] <- matrix(sqrt(pmax(z_var_vec, 0)), nrow = nx, ncol = ny)
}
...
p_post_low_CI <- apply(p_draws, MARGIN = c(1, 2, 3), FUN = function(x) {
  quantile(x, probs = 0.025)
})
p_post_up_CI <- apply(p_draws, MARGIN = c(1, 2, 3), FUN = function(x) {
...
model_output <- list(
  ...
  p_post_CI = p_post_CI,
  p_post_low_CI = p_post_low_CI, 
  p_post_up_CI = p_post_up_CI, 
  z_post_sd = z_post_sd_analytic,
