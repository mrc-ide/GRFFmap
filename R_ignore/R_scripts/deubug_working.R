...
K_init <- sigma2_init * exp(-(dist_mat^2) / (2 * ell_sim^2))
K_rw   <- sigma2_rw   * exp(-(dist_mat^2) / (2 * ell_sim^2))

diag(K_init) <- diag(K_init) + 1e-8
diag(K_rw)   <- diag(K_rw) + 1e-8

L_init <- t(chol(K_init))
L_rw   <- t(chol(K_rw))

# --- Generate latent field over time -------------------------------------
z_true_array <- array(NA_real_, dim = c(nx_sim, ny_sim, T_years))
p_true_array <- array(NA_real_, dim = c(nx_sim, ny_sim, T_years))

# Initial year
z_prev <- -1 + as.numeric(L_init %*% rnorm(M))

for (tt in seq_len(T_years)) {
  if (tt > 1) {
...
tt_true <- which(years == 2021)

plot_df <- data.frame(
  x = grid_ll$lon,
  y = grid_ll$lat,
  p_true = as.vector(p_true_array[, , tt_true]),
  z_true = as.vector(z_true_array[, , tt_true]),
  p_mean = as.vector(p_post_mean[, , t_idx]),
  p_lo = as.vector(p_post_lower[, , t_idx]),
  p_hi = as.vector(p_post_upper[, , t_idx])
)

plot_df$p_width <- plot_df$p_hi - plot_df$p_lo
plot_df$covered <- with(plot_df, p_true >= p_lo & p_true <= p_hi)

S_grid <- as.matrix(grid_ll[, c("lon", "lat")])
S_obs  <- as.matrix(dat[, c("longitude", "latitude")])

dmat <- as.matrix(dist(rbind(S_grid, S_obs)))
n_grid <- nrow(S_grid)
n_obs  <- nrow(S_obs)

plot_df$d_to_obs <- apply(dmat[1:n_grid, (n_grid + 1):(n_grid + n_obs), drop = FALSE], 1, min)

ggplot(plot_df, aes(x = x, y = y, fill = p_true)) +
  geom_raster() +
  coord_fixed() +
  scale_fill_viridis_c(name = "True p") +
  ggtitle("True prevalence")

ggplot(plot_df, aes(x = x, y = y, fill = p_mean)) +
  geom_raster() +
  geom_point(data = dat, aes(x = longitude, y = latitude),
             inherit.aes = FALSE, color = "red", size = 1.5) +
  coord_fixed() +
  scale_fill_viridis_c(name = "Posterior mean") +
  ggtitle("Posterior mean prevalence")

ggplot(plot_df, aes(x = x, y = y, fill = p_width)) +
  geom_raster() +
  geom_point(data = dat, aes(x = longitude, y = latitude),
             inherit.aes = FALSE, color = "red", size = 1.5) +
  coord_fixed() +
...
ggplot(plot_df, aes(x = x, y = y, fill = covered)) +
  geom_raster() +
  coord_fixed() +
  scale_fill_manual(values = c("FALSE" = "black", "TRUE" = "gold")) +
  ggtitle("Coverage map: true p inside 95% interval")
