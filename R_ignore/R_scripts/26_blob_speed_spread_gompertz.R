...
OUT_PLOT_DIR       <- "largest_blob_speed_gompertz"
...
for (mut in all_who_mutations){
  ...
  exceed_prob_exceed <- cached$exceed_post_draw_list
  exceed_prob_exceed_1 <- exceed_prob_exceed$`1`
  exceed_prob_exceed_5 <- exceed_prob_exceed$`5`
  exceed_prob_exceed_10 <- exceed_prob_exceed$`10`
  
  xs <- cached$xs
  ys <- cached$ys
  
  exceed_prob_long_10 <- make_raster_long(exceed_prob_exceed_10, xs, ys, plot_times)
  exceed_prob_long_5 <- make_raster_long(exceed_prob_exceed_5, xs, ys, plot_times)
  exceed_prob_long_1 <- make_raster_long(exceed_prob_exceed_1, xs, ys, plot_times)
  ...
  for (p_thr in c(0.01, 0.05, 0.10)) {
    ...
    model_fits_exceed[[key]] <- fit_gomp
  }
  ...
  gomp_model_fit <- ggplot() +
    ...
    labs(
      ...
      title = "Gompertz Model fit for largest blob across time"
    ) +
    theme_bw() +
    theme(
  ...
  save_figs(file.path(OUT_PLOT_DIR, paste0(mut, flag, "_speed_spread_time_largest_blob_gomp")), gomp_model_fit, width = 10)
  }
