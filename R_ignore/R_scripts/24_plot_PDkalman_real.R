...
# plotting parameters
plot_times <- seq(2002, 2023, by = 2) # should be within t_vec
...
for (mut in all_who_mutations){
  ...
  if (dim(pts_pos)[1] != 0){
    ...
    exceed_prob_long_50 <- mask_to_africa(exceed_prob_long_50, africa_mask)
    exceed_prob_long_20 <- mask_to_africa(exceed_prob_long_20, africa_mask)
...
#plot_mean_no_title  <- plot_layer(p_long_mean, mut, shape_Africa_crop, shape_water_crop, add_points = TRUE, add_legend = TRUE)
#save_figs(file.path(OUT_PREV_DIR, paste0(mut, "_mean_perc_prev_p_init0.07")), plot_mean_no_title, width = 10, svg_plot = FALSE)
...
#plot_exceed_50 <- plot_exceedance(exceed_prob_long_50, mut, legend_title =expression(Pr(Prevalence >= 50*"%")))
#save_figs(file.path(OUT_EXCEED_DIR, paste0(mut, "_exceedance_prob_50")), plot_exceed_50, width = 10, svg_plot = FALSE)
