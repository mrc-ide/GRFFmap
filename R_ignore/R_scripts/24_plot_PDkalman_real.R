...
# plotting parameters
plot_times <- seq(2012, 2023, by = 2) # should be within t_vec
...
OUT_PREV_DIR_DRAWS  <- file.path("prev_prediction_grouped_year_kalman_draws")
OUT_PREV_DIR_LO_UP_DRAWS <- file.path("prev_prediction_grouped_year_kalman_draws/upper_lower_prev")
OUT_EXCEED_DIR_DRAWS <- file.path("exceedance_prob_grouped_year_kalman_draws")
dir_create(c(paste0("R_ignore/R_scripts/outputs/plots/", OUT_PREV_DIR), 
             paste0("R_ignore/R_scripts/outputs/plots/", OUT_PREV_DIR_LO_UP),
             paste0("R_ignore/R_scripts/outputs/plots/", OUT_EXCEED_DIR),
             paste0("R_ignore/R_scripts/outputs/plots/", OUT_PREV_DIR_DRAWS), 
             paste0("R_ignore/R_scripts/outputs/plots/", OUT_PREV_DIR_LO_UP_DRAWS),
             paste0("R_ignore/R_scripts/outputs/plots/", OUT_EXCEED_DIR_DRAWS)), 
           recurse = TRUE)

dat <- read.csv("R_ignore/R_scripts/data/partner_drug_get_prevalence.csv") |>
...
# --------------------------- Define Mutations ----------------------------
all_who_mutations <- c("k13:comb", "k13:622:I", "k13:469:Y", "k13:675:V", "k13:446:I", "k13:458:Y", "k13:476:I",   "k13:493:H",   "k13:539:T",
...
for (mut in all_who_mutations){
 ...
 #   exceed_prob <- cached$exceed_post_gaussian_approx_list
    ...
    #p_long_lo   <- mask_to_africa(p_long_lo)
    #p_long_hi   <- mask_to_africa(p_long_hi)
    ...
    plot_layer <- function(p_long_df, title_text, shp = shape_Africa, shp_water = shape_water, add_points = FALSE, add_legend = FALSE) {
      p <- ggplot() +
        ...
        geom_sf(data = shp_water, fill = "black", colour = NA) +
        coord_sf(
      ...
      if (add_points) {
        p <- p + geom_point(
          ...
          shape = 21, colour = "lightgrey", size = 1.5, stroke = 0.2
        )
      }
      if (!add_legend){
      ...
      p <- p + scale_fill_gradientn(colours = pp_cols,
                                    ...
                                    limits = c(0, 99), 
                                    name = "Prevalence (%)",
                                    breaks = seq(0, max(pp_vals), by = 10),
                                    na.value = "black") +
        facet_wrap(~ t, nrow = 2) +
    ...
    plot_exceedance <-function(exceed_prob, title_text, legend_title, shp = shape_Africa, shp_water = shape_water, add_points = FALSE, add_legend = FALSE) {
      p <- ggplot() +
        ...
        #   values = rescale(c(0, 0.2, 0.8, 1)),  # Low–Mid–High emphasis
        #   na.value = "white"
        # ) +
    ...
    #plot_lower_no_title <- plot_layer(p_long_lo, "", shape_Africa, shape_water, add_points = TRUE,  add_legend = TRUE)
    plot_mean_no_title  <- plot_layer(p_long_mean, "", shape_Africa_crop, shape_water_crop, add_points = TRUE, add_legend = TRUE)
    plot_mean_draws_no_title  <- plot_layer(p_draws_long_mean, "", shape_Africa_crop, shape_water_crop, add_points = TRUE, add_legend = TRUE)
    #plot_upper_no_title <- plot_layer(p_long_hi, "", shape_Africa, shape_water, add_points = TRUE, add_legend = TRUE)
    
    #plot_lower_no_title_no_points <- plot_layer(p_long_lo, "", shape_Africa, shape_water, add_points = FALSE,  add_legend = TRUE)
    ...
    #plot_upper_no_title_no_points <- plot_layer(p_long_hi, "", shape_Africa, shape_water, add_points = FALSE, add_legend = TRUE)
