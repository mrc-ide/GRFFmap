...
phi_of_coords <- function(S_km) {
  OS <- as.matrix(S_km) %*% t(Omega)
  Phi <- cbind(cos(OS), sin(OS)) * (1 / sqrt(D))
  cbind(1, Phi)
}
...
Phi_full <- cbind(1, cbind(cos(OS_full), sin(OS_full)) * (1 / sqrt(D)))
...
for (ti in 1:t_num) {
  ...
  if (nrow(dat_t) > 0) {
    ...
    mu_t <- rep(0, length(y_t))             # constant logit mean
...
# C0_w <- Diagonal(2 * D, 1.0)  # prior covariance (zero variance, i.e. initial frequency known exactly)

Qw <- Diagonal(1 + 2 * D, c(0.2, rep(tau2, 2 * D)))
m_w  <- matrix(0, 1 + 2 * D, t_num)
m0_w <- c(0, rep(0, 2 * D))   # prior median prevalence 0.5 on logit scale
C0_w <- Diagonal(1 + 2 * D, c(9, rep(2, 2 * D)))


# --------------------------- EM loop ---------------------------------------
m_pred <- vector("list", t_num)
C_pred <- vector("list", t_num)
m_filt <- matrix(NA, 1 + 2 * D, t_num)
...
for (i in 1:max_iter) {
  ...
  if (TRUE) {
    ...
    for (k in seq_along(plot_times)) {
      ...
      # Mean z on grid
      z_mean_vec <- as.numeric(Phi_full %*% m_w[, t_idx])
      ...
      # Store as [nx x ny]
      z_post_mean[, , t_idx] <- matrix(z_mean_vec, nrow = nx, ncol = ny, byrow = FALSE)
      p_post_mean[, , t_idx] <- matrix(p_mean_vec, nrow = nx, ncol = ny, byrow = FALSE)
      z_post_sd[,   , t_idx] <- matrix(z_sd_vec,   nrow = nx, ncol = ny, byrow = FALSE)
      p_post_lo[,   , t_idx] <- matrix(p_lo_vec,   nrow = nx, ncol = ny, byrow = FALSE)
      p_post_hi[,   , t_idx] <- matrix(p_hi_vec,   nrow = nx, ncol = ny, byrow = FALSE)
    }
...
beta_draws <- array(NA_real_, dim = c(2 * D + 1, t_num, num_post_draws))
...
for (b in 1:num_post_draws) {
  ...
  # --------------------------------------------
  beta_b <- matrix(NA_real_, nrow = 2 * D + 1, ncol = t_num)
  ...
  for (ti in (t_num - 1):1) {
    ...
    Var_t <- forceSymmetric(P_t - J_t %*% R_tp1 %*% t(J_t))
    eig <- eigen(Var_t, symmetric = TRUE)
    eig$values[eig$values < 1e-10] <- 1e-10
    Var_t <- eig$vectors %*% diag(eig$values) %*% t(eig$vectors)
    
    # Draw β_t ~ N(mean_t, Var_t)
    # Rch_t <- chol(Var_t)
    Rch_t <- chol(forceSymmetric(Var_t))
    eta_t <- rnorm(2 * D + 1)
  ...
  # --------------------------------------------
  for (ti in 1:t_num) {
    z_vec <- as.numeric(Phi_full %*% beta_b[, ti])   # length M = nx*ny
...
p_lower_CI <- make_raster_long(p_post_lower[ , , plot_idx, drop = FALSE], xs, ys, plot_times)
p_upper_CI <- make_raster_long(p_post_upper[ , , plot_idx, drop = FALSE], xs, ys, plot_times)
...
ggplot() + 
  geom_raster(data = p_lower_CI %>% filter(t==2021), aes(x=x, y=y, fill = p*100)) + 
  geom_point(
    data = dat_plot,
  ...
  coord_fixed(xlim = c(lon_min, lon_max), ylim = c(lat_min, lat_max), expand = FALSE) +
  scale_fill_viridis_c(limits = c(0, 100)) + ggtitle("Lower from Draws")

ggplot() + 
  geom_raster(data = p_upper_CI %>% filter(t==2021), aes(x=x, y=y, fill = p*100)) + 
  geom_point(
    data = dat_plot,
    aes(x = longitude, y = latitude),
    inherit.aes = FALSE,
  ...
  coord_fixed(xlim = c(lon_min, lon_max), ylim = c(lat_min, lat_max), expand = FALSE) +
  scale_fill_viridis_c(limits = c(0, 100)) + ggtitle("Upper from Draws")

ggplot() + geom_raster(data = p_diff_CI %>% filter(t==2021), aes(x=x, y=y, fill = p*100)) + 
  geom_point(
    data = dat_plot,
    aes(x = longitude, y = latitude),
    inherit.aes = FALSE,
  ...
  coord_fixed(xlim = c(lon_min, lon_max), ylim = c(lat_min, lat_max), expand = FALSE) +
  scale_fill_viridis_c(limits = c(0, 100)) + ggtitle("Diff Draws")

p_post_lo_long <- make_raster_long(p_post_lo[ , , plot_idx, drop = FALSE], xs, ys, plot_times)
p_post_hi_long <- make_raster_long(p_post_hi[ , , plot_idx, drop = FALSE], xs, ys, plot_times)

ggplot() + geom_raster(data = p_post_lo_long %>% filter(t==2021), aes(x=x, y=y, fill = p*100)) + scale_fill_viridis_c(limits = c(0, 100)) + ggtitle("Lower from Approx")
ggplot() + geom_raster(data = p_post_hi_long %>% filter(t==2021), aes(x=x, y=y, fill = p*100)) + scale_fill_viridis_c(limits = c(0, 100)) + ggtitle("Upper from Approx")

p_draws_29 <- p_draws[ , , 29, ]

p_post_sd <- apply(p_draws_29, c(1, 2), sd, na.rm = TRUE)
plot(p_post_sd)
p_post_range <- apply(p_draws_29, c(1, 2), function(x) {
  max(x, na.rm = TRUE) - min(x, na.rm = TRUE)
})

p_post_iqr <- apply(p_draws_29, c(1, 2), function(x) {
